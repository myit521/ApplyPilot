...
在工作流状态这一层，PostgreSQL 内部再分两个角色：LangGraph checkpoint 表是节点级状态的**事实源**（中断、恢复、重放都以它为准）；`workflow_runs` 表是应用层维护的**查询索引**，只服务列表页和状态查询。恢复逻辑不得读取 `workflow_runs`。
...
- `resume_versions`：目标职位、分区结构的生成内容（见 8.3）、提示词版本、审批状态和创建时间。
- `resume_claims`：简历中的单条表述以及其引用的事实 ID。
- `workflow_runs`：工作流运行的**查询索引**（当前节点、状态、重试次数、输入摘要和错误信息）。工作流状态的事实源是 LangGraph checkpoint 表；`workflow_runs` 只保存供列表和状态页查询的镜像摘要，不参与恢复决策，允许与 checkpoint 短暂不一致。
...
1. 使用事实启用状态（`enabled`）做硬过滤。MVP 只实现这一条元数据过滤：事实库为单一候选人服务，毕业时间、经历类型等过滤在当前数据规模下没有区分度。当评测集中出现带明确毕业时间或经历类型约束的 JD 时，再把对应过滤加回硬过滤层。
2. 使用 PostgreSQL 全文检索匹配明确技能词和业务词。
3. 使用 pgvector 召回语义相关事实，并与全文结果合并去重。JD 关键词与事实技能标签的词表经常不重叠（如 JD 写 Java/MySQL，事实标签是"批量执行/异步任务"），此时全文检索得分趋近于零，语义召回是主要通路，嵌入模型为本地 bge-small-zh（512 维）。

默认返回前 5 条候选事实，同时保留各事实的检索来源和得分。MVP 不增加独立重排模型，使用必备技能覆盖、文本匹配和向量相似度的可解释加权分数。

### 8.3 简历生成节点

模型只能使用检索节点提供的事实生成内容。生成结果按真实简历结构组织为分区（sections），而不是无结构的条目列表：

```json
{
  "sections": {
    "education": [{"text": "武汉轻工大学 软件工程 本科（2027 届）", "fact_ids": ["fact_edu_01"]}],
    "skills": [{"text": "Java、Spring Boot、MySQL、Redis", "fact_ids": ["fact_intern_batch_01"]}],
    "experience": [
      {
        "text": "承担运维工具批量执行模块开发……",
...
`resume_versions.content` 保存上述分区结构作为版本事实源；DOCX 导出按分区渲染模板（标题、教育背景、技能、经历），而非简单的条目堆叠。`experience` 分区的每条输出沿用原有的 claim 结构（`text` + `fact_ids` + `matched_requirements`），事实校验规则不变；`education` 和 `skills` 分区同样必须携带事实引用，教育背景只允许来自 `education` 类型事实。
...
- 不允许把计划中、未验收的功能写成已完成。
- 规则通过后，再由模型检查语义是否超出事实含义。语义复核是单次模型判定，没有一致性投票机制；实践表明它能拦截明显的背景嫁接（如把商城项目的测试工作写成保障交易系统），但"提升效率"一类轻度发挥会被放行。边界样例不依赖该层现场判断，统一进入评测集人工标注（见 12.2），用事实一致率指标量化该层表现。
...
- 服务重启：从 LangGraph checkpoint 表中最后一个成功节点恢复。
...
- 不应出现在简历中的事实。
- 允许使用的量化指标。
- 语义边界样例：对每条 JD 准备若干"轻度发挥"和"明显越界"的表述，标注是否应被语义复核层拦截，用于量化该层的误判与漏判。
